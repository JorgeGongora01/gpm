...
class WC_Woocommerce_Catalog_Enquiry_Frontend {
	...
	public function add_form_for_enquiry_without_popup() {		
		...
		<?php if(isset($custom_design_for_button) && $custom_design_for_button == "Enable" ) {?>
			<br/>
			<button class="woo_catalog_enquiry_custom_button_enquiry"  onclick="do_toggle();" href="#responsive"><?php echo $button_text;?></button>
			<?php
		}
		else {?>
		<button class="demo btn btn-primary btn-large" style="margin-top:15px;" onclick="do_toggle();" href="#responsive"><?php echo __('Send an enquiry',$WC_Woocommerce_Catalog_Enquiry->text_domain)?></button>
		<?php }?>
			...
			<div class="modal-body">
				<div class="row-fluid">
					<div class="span12">
						...
						<p><?php if( isset($settings['fileupload_label']) && $settings['fileupload_label'] != '' && $settings['fileupload_label'] !=' ') { echo $settings['fileupload_label']; } else { echo __('Upload your File : ',$WC_Woocommerce_Catalog_Enquiry->text_domain); } ?></p>	
	...
	public function add_form_for_enquiry() {		
		...
		<div id="responsive"  class="catalog_modal">
			<div class="modal-content">
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-default"><?php echo __('Close',$WC_Woocommerce_Catalog_Enquiry->text_domain);?></button>
					<button onclick="submitthis('frm_woo_catalog');" type="button" id="woo_submit_enquiry" class="btn btn-primary"><?php echo __('Send', $WC_Woocommerce_Catalog_Enquiry->text_domain);?></button>
				</div>
			</div>
		</div>			
	...
	function frontend_scripts() {
		...
		$settings = $WC_Woocommerce_Catalog_Enquiry->options;	
     	...
     	if( isset($settings['load_js_lib']) && $settings['load_js_lib'] == "Enable") {
     		wp_enqueue_script('jquerymin_js', 'http://ajax.googleapis.com/ajax/libs/jquery/1.8.2/jquery.min.js', array(), false, false);
     	}
     	if(isset($settings['is_enable']) && $settings['is_enable'] == "Enable"){
			...
			// error levels
			$error_levels = array();
			$error_levels['name_required'] = __('Name is required field',$WC_Woocommerce_Catalog_Enquiry->text_domain);
			$error_levels['email_required'] = __('Email is required field', $WC_Woocommerce_Catalog_Enquiry->text_domain);
			$error_levels['email_valid'] = __('Please Enter Valid Email Id', $WC_Woocommerce_Catalog_Enquiry->text_domain);
			$error_levels['captcha_required'] = __('Please enter the security code',$WC_Woocommerce_Catalog_Enquiry->text_domain);
			$error_levels['captcha_valid'] = __('Please enter the valid seurity code',$WC_Woocommerce_Catalog_Enquiry->text_domain);
			$error_levels['ajax_error'] = __('Error in system please try later',$WC_Woocommerce_Catalog_Enquiry->text_domain);
			...
			array(
				...
				'ajax_success_msg' 		=> __('Enquiry sent successfully',$WC_Woocommerce_Catalog_Enquiry->text_domain),
