...
class WC_Woocommerce_Catalog_Enquiry_Admin {
  ...
  public $settings;
    ...
    <div id="dc_admin_footer">
      <?php _e('Powered by', $WC_Woocommerce_Catalog_Enquiry->text_domain); ?> <a href="http://dualcube.com" target="_blank"><img src="<?php echo $WC_Woocommerce_Catalog_Enquiry->plugin_url.'/assets/images/dualcube.png'; ?>"></a><?php _e('Dualcube', $WC_Woocommerce_Catalog_Enquiry->text_domain); ?> &copy; <?php echo date('Y');?>
    </div>
    <?php
	}
	...
	public function save_catalog_data( $post_id ) {
		...
		// Save all meta
		update_post_meta( $post_id, 'woo_catalog_enquiry_product_link', $_POST['woo_catalog_enquiry_product_link'] );		
	...
	public function catalog_product_data_panel() {
		...
		?><div id="catalog_enquiry_product_data" class="panel woocommerce_options_panel"><?php			
			...
			woocommerce_wp_text_input( array(
				...
				'label' 	=> __( 'Enter product external link', $WC_Woocommerce_Catalog_Enquiry->text_domain ),
				'placeholder' => __('https://www.google.com', $WC_Woocommerce_Catalog_Enquiry->text_domain)
			) );	
