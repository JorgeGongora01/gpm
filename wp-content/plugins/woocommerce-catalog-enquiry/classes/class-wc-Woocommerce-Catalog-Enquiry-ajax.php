...
class WC_Woocommerce_Catalog_Enquiry_Ajax {
	...
	public function send_product_enqury_mail() {
		...
		$subject_mail = __('Product Enquiry for ',$WC_Woocommerce_Catalog_Enquiry->text_domain).$product_name.' by '.$name;
...
<?php		
		...
		if(wp_mail( $email_admin, $subject_mail, $email_content, $headers, $attachments )) {
			echo 1;
